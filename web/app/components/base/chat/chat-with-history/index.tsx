...
const ChatWithHistory: FC<ChatWithHistoryProps> = ({ className }) => {
  ...
  return (
    <div
      className={cn('flex h-full bg-white ', isMobile && 'flex-col', className)}
    >
      {!isMobile && (
        <div
          className={cn(
            'flex w-[300px] flex-col p-1 pr-0 transition-all duration-200 ease-in-out ',
      ...
      >
        {isSidebarCollapsed && (
          <div
            className={cn(
              'absolute top-0 z-20 flex h-full w-[300px] flex-col p-2 transition-all duration-500 ease-in-out',
              showSidePanel ? 'left-0' : 'left-[-300px]',
            )}
        ...
        <div
          className={cn(
            'flex h-full flex-col overflow-hidden rounded-none border-[0,5px] border-components-panel-border-subtle bg-chatbot-bg',
          )}
        >
          {!isMobile && <Header />}
