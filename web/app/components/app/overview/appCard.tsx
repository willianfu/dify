...
function AppCard({
  ...
  const currentUrl = useMemo(() => {
    if (!isApp)
      return appInfo?.api_base_url

    return `${window.location.protocol}//${window.location.host}/${appMode}/${access_token}`
  }, [isApp, appInfo])
  ...
  return (
    <div
      className={`${
        isInPanel ? 'border-l-[0.5px] border-t' : 'border-[0.5px] shadow-xs'
      } w-full max-w-full rounded-xl border-effects-highlight ${
        className ?? ''
      }`}
    >
        ...
        <div className="flex w-full flex-col items-start justify-center gap-3 self-stretch border-b-[0.5px] border-divider-subtle p-3">
          <div className="flex w-full items-center gap-3 self-stretch">
            ...
            <div className="flex items-center gap-1">
              ...
              <div
                className={`${
                  runningStatus ? 'text-text-success' : 'text-text-warning'
          ...
          <div className="flex flex-col items-start justify-center self-stretch">
            ...
            <div className="inline-flex h-9 w-full items-center gap-0.5 rounded-lg bg-components-input-bg-normal p-1 pl-2">
              ...
              <CopyFeedback content={currentUrl} className={'!size-6'} />
              {isApp && (
                <ShareQRCode
                  content={currentUrl}
                  className="z-50 !size-6 rounded-md hover:bg-state-base-hover"
              ...
              {isApp && isCurrentWorkspaceManager && (
                <Tooltip
                  popupContent={
                    t('appOverview.overview.appInfo.regenerate') || ''
                  }
                >
                  <div
